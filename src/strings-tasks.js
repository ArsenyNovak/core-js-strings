...
function removeFirstOccurrences(str, value) {
  ...
  return str.slice(0, indexstart) + str.slice(indexend, str.length);
}
...
function isPalindrome(str) {
  let newstr = '';
  const row1 = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  for (let j = 0; j < str.length; j += 1) {
    if (row1.includes(str[j])) {
      newstr += str[j];
    }
  }
  newstr = newstr.toLowerCase();
  const end = newstr.length;
  for (let i = 0; i < end / 2; i += 1) {
    if (newstr[i] !== newstr[end - i - 1]) {
      return false;
    }
  }
  return true;
...
function findLongestWord(sentence) {
  ...
  for (let i = 0; i < sentence.length; i += 1) {
    if (sentence[i] === ' ' || i === sentence.length - 1) {
      if (maxword.length < i - start) {
        maxword = sentence.slice(start, i);
      }
      start = i + 1;
    }
  }
...
function reverseWords(str) {
  ...
  for (let i = 0; i < massiv.length; i += 1) {
    rvsstr = '';
    for (let j = massiv[i].length - 1; j >= 0; j -= 1) {
      rvsstr += massiv[i][j];
    }
    massiv[i] = rvsstr;
  }
...
function invertCase(str) {
  const massiv = str.split('');
  for (let i = 0; i < massiv.length; i += 1) {
    if (massiv[i].charCodeAt() >= 97 && massiv[i].charCodeAt() <= 122) {
      massiv[i] = massiv[i].toUpperCase();
    } else if (massiv[i].charCodeAt() >= 65 && massiv[i].charCodeAt() <= 90) {
...
function encodeToRot13(str) {
  const rot1 = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  const rot13 = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  let newstr = '';
  for (let i = 0; i < str.length; i += 1) {
    if (rot1.includes(str[i])) {
      newstr += rot13[rot1.search(str[i])];
    } else {
      newstr += str[i];
    }
  }
